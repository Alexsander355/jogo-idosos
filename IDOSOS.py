...
def criar_tela(tipo, titulo, largura):
    ...
    Button(
        tela,
        text="Confirmar",
        command=lambda: conferir_resposta(caixa.get(), resposta)
    ).pack(pady=10)
...
def iniciar_programa():
    ...
    janela_principal.geometry("320x400")
    ...
    # Frame das atividades
    frame_atividades = Frame(janela_principal)
    frame_atividades.pack(pady=10)

    # FUNÇÃO DAS DIFICULDADES
    def selecionar_dificuldade(nivel):
        nonlocal frame_atividades

        # Remove o frame antigo inteiro
        frame_atividades.destroy()

        # Cria um novo frame vazio
        frame_atividades = Frame(janela_principal)
        frame_atividades.pack(pady=10)
    ...
    Button(
        ...
        text="Fácil",
        width=15,
        command=lambda: selecionar_dificuldade("facil")
    ).pack(pady=3)
    ...
    Button(
        ...
        text="Normal",
        width=15,
        command=lambda: selecionar_dificuldade("normal")
    ).pack(pady=3)
    ...
    Button(
        ...
        text="Difícil",
        width=15,
        command=lambda: selecionar_dificuldade("dificil")
    ).pack(pady=3)
